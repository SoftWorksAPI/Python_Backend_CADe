...
async def _validate_and_read(
    ...
    """Valida o upload e retorna o conteudo em bytes."""
...
# ---------------------------------------------------------------------------
# GET /v1/ai/health — Testar se a IA está online
# ---------------------------------------------------------------------------

@router.get(
    "/ai/health",
    summary="Testa se a IA está online e respondendo",
    description=(
        "Envia uma mensagem simples ao LLM via OpenRouter para verificar "
        "se a API está funcionando. Retorna o modelo configurado e a resposta."
    ),
)
...
# ---------------------------------------------------------------------------
# POST /v1/extract/dxf — Pipeline completo COM IA (Memorial Descritivo)
# ---------------------------------------------------------------------------
...
@router.post(
    ...
    description=(
        "Recebe um arquivo DXF, extrai dados deterministicamente com ezdxf, "
        "envia ao Gemini para analise como engenheiro auditor, e retorna o "
        "Memorial Descritivo estruturado em JSON com nivel de confianca e "
...
@router.post(
    ...
    description=(
        "Recebe um arquivo DXF e extrai todos os dados deterministicamente: "
        "elementos geometricos, blocos, textos, cotas, leaders, hatches e "
        "ambientes. Retorna os dados estruturados + prompt pronto para IA."
    ),
)
...
# ---------------------------------------------------------------------------
# POST /v1/extract/dxf/prompt — Apenas o prompt de IA
# ---------------------------------------------------------------------------
...
@router.post(
    ...
    description=(
        "Recebe um arquivo DXF, extrai os dados e retorna apenas a string "
        "do prompt que seria enviada ao LLM. Util para inspecao manual."
    ),
)
